...
1. افتح `/register` وأنشئ مستخدمًا جديدًا مع اختيار الدور `HR_MANAGER` أو `HR_EMPLOYEE`.
2. من قاعدة بيانات MySQL حدّث حقل `status` لهذا المستخدم إلى `ACTIVE` (مثلًا عبر أمر SQL: `UPDATE employee SET status='ACTIVE' WHERE email='example@iti.eg';`).
3. ادخل إلى `/admin/login` بنفس البريد وكلمة المرور للوصول إلى لوحة التحكم الإدارية وضبط بقية الحسابات والأدوار.
